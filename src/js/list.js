...
$.ajax({
  ...
  success:function(res){
    ...
    $('.pagi').pagination({
      ...
      pageCount: Math.ceil(res.length/6),//总页数
      current:1,//当前页
      jump: true,//是否开启跳转指定页
      coping: true,
      homePage: '首页',//首页文本
      ...
      callback: function (api) {//切换页面的时候会触发
        //api.getCurrent()获取当前是第几页
          ...
          //根据第几页从总数组里面筛选出一部分数据
          let list=res.slice((currPage-1)*6,currPage*6)//slice包前不包后currPage*15-1+1
          // console.log(list);   
...
//再次重新渲染页面
    ...
    $('.pagi').pagination({
      // mode:"fixed",//fixed固定分页器数量
      pageCount: Math.ceil(list1.length/6),//总页数
      current:1,//当前页
      jump: true,//是否开启跳转指定页
      coping: true,
      homePage: '首页',//首页文本
      ...
      callback: function (api) {//切换页面的时候会触发
        //api.getCurrent()获取当前是第几页
          ...
          //根据第几页从总数组里面筛选出一部分数据
          let list=list1.slice((currPage-1)*6,currPage*6)//slice包前不包后currPage*15-1+1
          console.log(list);   
